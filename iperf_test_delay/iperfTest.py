    ...
    def build(self, n=2, _delay="5ms"):
        ...
        for h in range(n):
            ...
            self.addLink(host, switch,
                         bw=1000, delay=_delay, use_htb=True)
...
for delay in [5,10,20,50,100,200]:
    info('-------------------------------------\n')
    info('-------------- '+dt.datetime.now().strftime('%X')+' -------------\n')
    info('   '+str(delay)+'ms delay on all links\n')
    info('-------------------------------------\n')
    perfTest(str(delay)+'ms')
